...
/**
 * 상점 메인 모바일 뷰 controller.
 * 생성 : 2018-12-14 (금) 15:05:57 
	...
	public function _remap($method)
 	{
		...
		if($method=="write"||$method=="modify")
		{
			...
			//푸터 include		
			$this->load->view('footer_page_write_v');
		}else{
			//헤더 include
//			$this->load->view('page/header_v');
			//
			if( method_exists($this, $method) )
			{
				//$this->{"{$method}"}();
...
//		$this->output->enable_profiler(FALSE);
		...
		'total_rows' => $this->mobile_m->get_list($this->uri->segment(3), 'count', '', '', $search_word),
		'per_page' => 5,
...
//		'cur_tag_open'	=> '<li class="disabled"><a href="#">',
		...
		$data['list'] = $this->mobile_m->get_list($this->uri->segment(3), '', $start, $limit, $search_word);
		$this->load->view('mobile/list_v', $data);
	}
...
//		$board_id = $this->uri->segment(5);
		...
		$mobile=$this->uri->segment(3)=="mobile";
 		...
 		$data['views'] = $this->mobile_m->get_view($table, $board_id);

 		//원산지 정보 가져오기 
 		$data['origin'] = $this->mobile_m->get_origin($board_id);
		//print_r($data['origin']);
		//게시판 이름과 게시물 번호에 해당하는 댓글 리스트 가져오기
 		//$data['comment_list'] = $this->mobile_m->get_comment($table, $board_id);
		...
		$this->load->view('page/view2_v', $data);
			...
			if($isMobile||$mobile)
			{
				...
				$this->load->view('page/pcview_v', $data);
			}
		}
 	}
...
//					'st_closingdate' => $this->input->post('st_closingdate', TRUE),
				...
				$result = $this->mobile_m->insert_store($write_data);
	...
	function modify()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			$writer_id = $this->mobile_m->writer_check($this->uri->segment(3), $this->uri->segment(5));
/*
			if( $writer_id->user_id != $this->session->userdata('username') )
			{
				alert('본인이 작성한 글이 아닙니다.', '/prq/store/view/'.$this->uri->segment(3).'/page_id/'.$this->uri->segment(5).'/page/'.$pages);
				exit;
			}
*/
			//폼 검증 라이브러리 로드
...
//				if ( !$this->input->post('mb_id', TRUE) AND !$this->input->post('mb_addr1', TRUE) )
				...
				$result = $this->mobile_m->modify_distributors($modify_data);
*/
				$modify_data = array(
					'table' => $this->uri->segment(3), //게시판 테이블명
...
//				$result = $this->distributors_m->insert_distributors($write_data);
				...
				$result = $this->mobile_m->modify_store($modify_data);
			...
			{
				//게시물 내용 가져오기
				$data['views'] = $this->mobile_m->get_view($this->uri->segment(3), $this->uri->segment(5));
		...
		{
			alert('로그인후 수정하세요', '/ttc/auth/login/');
			exit;
		}
 	}
	...
	function delete()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			$writer_id = $this->mobile_m->writer_check($table, $board_id);
			...
			//게시물 번호에 해당하는 게시물 삭제
			$return = $this->mobile_m->delete_content($this->uri->segment(3), $this->uri->segment(5));
