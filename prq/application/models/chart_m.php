...
//   		$this->db->query($sql0);
    	...
    	$sql = "SELECT DATE_FORMAT( cd_date,  '%Y-%m-%d' ) AS DATE, COUNT( * ) AS cnt
     FROM prq_cdr_tmp  where cd_id in ('0325536665@naver.com')
	 and cd_date like '2017-08%' 
 GROUP BY DATE_FORMAT( cd_date,  '%Y-%m-%d' )
 ORDER BY DATE_FORMAT( cd_date,  '%Y-%m-%d' ) ;";
   		$query = $this->db->query($sql);
...
/* Location: ./prq/application/models/chart_m.php */
