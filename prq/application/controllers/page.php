...
//		$this->output->enable_profiler(FALSE);
		...
		'total_rows' => $this->page_m->get_list($this->uri->segment(3), 'count', '', '', $search_word),
		'per_page' => 5,
...
//		'cur_tag_open'	=> '<li class="disabled"><a href="#">',
		...
		$data['list'] = $this->page_m->get_list($this->uri->segment(3), '', $start, $limit, $search_word);
...
//					'st_closingdate' => $this->input->post('st_closingdate', TRUE),
				...
				$result = $this->page_m->insert_store($write_data);
	...
	function modify()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			$writer_id = $this->page_m->writer_check($this->uri->segment(3), $this->uri->segment(5));
/*
...
//				$result = $this->distributors_m->insert_distributors($write_data);
				...
				$result = $this->page_m->modify_store($modify_data);
			...
			{
				//게시물 내용 가져오기
				$data['views'] = $this->page_m->get_view($this->uri->segment(3), $this->uri->segment(5));
	...
	function delete()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			$writer_id = $this->page_m->writer_check($table, $board_id);
			...
			//게시물 번호에 해당하는 게시물 삭제
			$return = $this->page_m->delete_content($this->uri->segment(3), $this->uri->segment(5));
