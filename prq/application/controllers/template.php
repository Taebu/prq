...
//		$this->output->enable_profiler(FALSE);
		...
		$config = array(
		//페이지네이션 기본 설정
		'base_url'=> '/prq/template/lists/bt_template'.$page_url.'/page/',
//		'total_rows' => $this->template_m->get_list("prq_blog", 'count', '', '', $search_word),
//		'total_rows' => $this->logs_m->get_list($this->uri->segment(3), 'count', '', '', $search_word),
		'total_rows' => $this->template_m->get_list2("bt_template", 'count', '', '', $search_array),
...
//		'cur_tag_open'	=> '<li class="disabled"><a href="#">',
		...
		//$data['list'] = $this->template_m->get_list("prq_blog", '', $start, $limit, $search_word);
		$data['list'] = $this->template_m->get_list2("bt_template", '', $start, $limit, $search_array);

		$this->load->view('template/list_v', $data);
	}
...
//		$board_id = $this->uri->segment(5);
		...
		$mobile=$this->uri->segment(3)=="mobile";
 		...
 		$data['views'] = $this->template_m->get_view($table, $board_id);
		//$array = json_decode(json_encode($data['views']),true);
		//$data['files'] = $this->template_m->get_files($array);
		//$data['store'] = $this->template_m->get_store($array);
		//$isMobile = $this->check_user_agent('mobile');
		//$array = json_decode(json_encode($data['store']),true);
		//$data['friends'] = $this->template_m->get_friends($array);
		$this->load->view('template/view_v', $data);
 	}
	...
	function write()
 	{
		...
		if(TRUE)
		{
			...
			if ( $this->form_validation->run() == TRUE )
			{
				...
				{
					//글 실패시 게시판 목록으로
					alert('다시 입력해 주세요.', '/prq/template/write/'.$this->uri->segment(3));
					exit;
				}
	...
	function modify()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE ||$is_bug)
		{
			...
			//$writer_id = $this->template_m->writer_check($this->uri->segment(3), $this->uri->segment(5));
/*
...
//				if ( !$this->input->post('mb_id', TRUE) AND !$this->input->post('mb_addr1', TRUE) )
        ...
        print_r($write_data);
				...
				{
					//글 수정 실패시 글 내용으로
					alert('다시 수정해 주세요.', '/prq/template/modify/'.$this->input->post('bl_no', TRUE).'/board_id/'.$this->uri->segment(5).'/page/'.$pages);
					exit;
				}
        ...
        print_r($write_data);
				...
				//member 정보 가져오기
				//$data['member'] = $this->template_m->get_member($arrays);


				//쓰기폼 view 호출
				$this->load->view('template/modify_v', $data);
			}
		}
		else
		{
...
//				if ( !$this->input->post('mb_id', TRUE) AND !$this->input->post('mb_addr1', TRUE) )
				...
				$result2 = $this->template_m->delete_file($write_data);
				...
				for($i=0;$i<count($img_src);$i++)
				{
					...
					//print_r($write_data);
					$result3 = $this->template_m->insert_file($write_data);
					echo $result3;
				} /*for($i=0;$i<=count($img_src);$i++){ ... } */
			...
			{
				//게시물 내용 가져오기
				$data['views'] = $this->template_m->get_view("prq_blog", $this->uri->segment(3));
				$array = json_decode(json_encode($data['views']),true);
				//파일 정보 가져오기
				$data['files'] = $this->template_m->get_files($array);
				...
				//쓰기폼 view 호출
				$this->load->view('template/modify2_v', $data);
			}
		}
		else
		{
	...
	function delete()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			$writer_id = $this->template_m->writer_check($table, $board_id);
			...
			//게시물 번호에 해당하는 게시물 삭제
			$return = $this->template_m->delete_content($this->uri->segment(3), $this->uri->segment(5));
...
//		$board_id = $this->uri->segment(5);
		...
		$this->load->view('template/view_custom_v', $data);
 	}
	...
	function writeone()
 	{
		...
		if(TRUE)
		{
			...
			{
				//쓰기폼 view 호출
				$this->load->view('template/writeone_v');	
			}
		}
		else
		{
	...
	function event()
 	{
		...
		if(TRUE)
		{
			...
			if ( $this->form_validation->run() == TRUE )
			{
				...
				{
					//글 실패시 게시판 목록으로
					alert('다시 입력해 주세요.', '/prq/template/event/'.$this->uri->segment(3));
					exit;
				}
			...
			else
			{
				//쓰기폼 view 호출
				//$this->load->view('template/write_v');	
				$this->load->view('template/event_v');	
			}
		}
		else
		{
