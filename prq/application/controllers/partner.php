	...
	function view()
 	{
		...
		$board_id = $this->uri->segment(5);
 		...
 		$data['views'] = $this->partner_m->get_view($table, $board_id);

		//게시판 이름과 게시물 번호에 해당하는 댓글 리스트 가져오기
 		$data['comment_list'] = $this->partner_m->get_comment($table, $board_id);

 		//view 호출
 		$this->load->view('partner/view_v', $data);
 	}
...
//				);
//				$mb_code = $this->partner_m->get_code($get_code_data);
...
//					'mb_code' => $this->input->post('mb_code', TRUE),
				...
				if ( $result )
				{
					...
					*/
					...
					$result = $this->partner_m->insert_code($write_data);
					//글 작성 성공시 게시판 목록으로
					alert('입력되었습니다.', '/prq/partner/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
				else
				{
					//글 실패시 게시판 목록으로
					alert('다시 입력해 주세요.', '/prq/partner/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
			...
			{
				//쓰기폼 view 호출
				$this->load->view('partner/write_v');
	...
	function modify()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			$writer_id = $this->partner_m->writer_check($this->uri->segment(3), $this->uri->segment(5));
/*
			if( $writer_id->user_id != $this->session->userdata('username') )
			{
				alert('본인이 작성한 글이 아닙니다.', '/prq/partner/view/'.$this->uri->segment(3).'/distributors_id/'.$this->uri->segment(5).'/page/'.$pages);
				exit;
			}
*/
			//폼 검증 라이브러리 로드
			...
			if ( $this->form_validation->run() == TRUE )
			{
				if ( !$this->input->post('mb_id', TRUE) AND !$this->input->post('mb_addr1', TRUE) )
				{
					...
					alert('비정상적인 접근입니다.', '/prq/partner/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
				...
				$result = $this->partner_m->modify_distributors($modify_data);
*/
				$modify_data = array(
					'table' => $this->uri->segment(3), //게시판 테이블명
...
//				$result = $this->distributors_m->insert_distributors($write_data);
				...
				{
					//글 작성 성공시 게시판 목록으로
					alert('수정되었습니다.', '/prq/partner/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
				else
				{
					//글 수정 실패시 글 내용으로
					alert('다시 수정해 주세요.', '/prq/partner/view/'.$this->uri->segment(3).'/board_id/'.$this->uri->segment(5).'/page/'.$pages);
					exit;
				}
	...
	function delete()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			$writer_id = $this->partner_m->writer_check($table, $board_id);

			if( $writer_id->user_id != $this->session->userdata('username') )
			{
				alert('본인이 작성한 글이 아닙니다.', '/prq/partner/view/'.$this->uri->segment(3).'/board_id/'.$this->uri->segment(5).'/page/'.$this->uri->segment(7));
				exit;
			}

			//게시물 번호에 해당하는 게시물 삭제
			$return = $this->partner_m->delete_content($this->uri->segment(3), $this->uri->segment(5));

			//게시물 목록으로 돌아가기
			if ( $return )
			{
				//삭제가 성공한 경우
				alert('삭제되었습니다.', '/prq/partner/lists/'.$this->uri->segment(3).'/page/'.$this->uri->segment(7));
			}
			else
			{
				//삭제가 실패한 경
				alert('삭제 실패하였습니다.', '/prq/partner/view/'.$this->uri->segment(3).'/board_id/'.$this->uri->segment(5).'/page/'.$this->uri->segment(7));
			}
		}
		else
		{
...
/* Location: ./prq/application/controllers/partner.php */