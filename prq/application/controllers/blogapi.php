...
//		$this->output->enable_profiler(FALSE);
		...
		$config = array(
		//페이지네이션 기본 설정
		'base_url'=> '/prq/blogapi/lists/prq_member'.$page_url.'/page/',
		'total_rows' => $this->blogapi_m->get_list($this->uri->segment(3), 'count', '', '', $search_word),
		'per_page' => 5,
		...
		$data['list'] = $this->blogapi_m->get_list($this->uri->segment(3), '', $start, $limit, $search_word);
		$data['count'] = json_decode(json_encode($this->blogapi_m->get_frcnt()), True);
		$data['controllers'] = $this;
		$this->load->view('blogapi/list_v', $data);
	}
	...
	function view()
 	{
		...
		$board_id = $this->uri->segment(5);
 		...
 		$data['views'] = $this->blogapi_m->get_view($table, $board_id);

		//게시판 이름과 게시물 번호에 해당하는 댓글 리스트 가져오기
 		$data['comment_list'] = $this->blogapi_m->get_comment($table, $board_id);

 		//view 호출
 		$this->load->view('blogapi/view_v', $data);
 	}
	...
	function write()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE  ||@$this->input->cookie('logged_in', TRUE) == TRUE)
		{
			...
			if ( $this->form_validation->run() == TRUE )
			{
				...
				{
					//글 작성 성공시 게시판 목록으로
					alert('입력되었습니다.', '/prq/blogapi/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
				else
				{
					//글 실패시 게시판 목록으로
					alert('다시 입력해 주세요.', '/prq/blogapi/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
			...
			{
				//쓰기폼 view 호출
				$this->load->view('blogapi/write_v');
			}
		}
		else
		{
	...
	function modify()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE  ||@$this->input->cookie('logged_in', TRUE) == TRUE)
		{
			...
			$writer_id = $this->blogapi_m->writer_check($this->uri->segment(3), $this->uri->segment(5));
/*
			if( $writer_id->user_id != $this->session->userdata('username') )
			{
				alert('본인이 작성한 글이 아닙니다.', '/prq/blogapi/view/'.$this->uri->segment(3).'/blogapi_id/'.$this->uri->segment(5).'/page/'.$pages);
				exit;
			}
*/
			//폼 검증 라이브러리 로드
			...
			if ( $this->form_validation->run() == TRUE )
			{
				if ( !$this->input->post('mb_id', TRUE) AND !$this->input->post('mb_addr1', TRUE) )
				{
					...
					alert('비정상적인 접근입니다.', '/prq/blogapi/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
				...
				$modify_data = array(
					...
					'blogapi_id' => $this->uri->segment(5), //게시물번호
					'subject' => $this->input->post('subject', TRUE),
					'contents' => $this->input->post('contents', TRUE)
				);

				$result = $this->blogapi_m->modify_blogapi($modify_data);
*/
				$modify_data = array(
					'table' => $this->uri->segment(3), //게시판 테이블명
...
//				$result = $this->blogapi_m->insert_blogapi($write_data);
				...
				{
					//글 작성 성공시 게시판 목록으로
					alert('수정되었습니다.', '/prq/blogapi/lists/'.$this->uri->segment(3).'/page/'.$pages);
					exit;
				}
				else
				{
					//글 수정 실패시 글 내용으로
					alert('다시 수정해 주세요.', '/prq/blogapi/view/'.$this->uri->segment(3).'/board_id/'.$this->uri->segment(5).'/page/'.$pages);
					exit;
				}
			...
			{
				//게시물 내용 가져오기
				$data['views'] = $this->blogapi_m->get_view($this->uri->segment(3), $this->uri->segment(5));

				//쓰기폼 view 호출
				$this->load->view('blogapi/modify_v', $data);
			}
		}
		else
		{
	...
	function delete()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE  ||@$this->input->cookie('logged_in', TRUE) == TRUE)
		{
			...
			$writer_id = $this->blogapi_m->writer_check($table, $board_id);

			if( $writer_id->user_id != $this->session->userdata('username') )
			{
				alert('본인이 작성한 글이 아닙니다.', '/prq/blogapi/view/'.$this->uri->segment(3).'/board_id/'.$this->uri->segment(5).'/page/'.$this->uri->segment(7));
				exit;
			}

			//게시물 번호에 해당하는 게시물 삭제
			$return = $this->blogapi_m->delete_content($this->uri->segment(3), $this->uri->segment(5));

			//게시물 목록으로 돌아가기
			if ( $return )
			{
				//삭제가 성공한 경우
				alert('삭제되었습니다.', '/prq/blogapi/lists/'.$this->uri->segment(3).'/page/'.$this->uri->segment(7));
			}
			else
			{
				//삭제가 실패한 경
				alert('삭제 실패하였습니다.', '/prq/blogapi/view/'.$this->uri->segment(3).'/board_id/'.$this->uri->segment(5).'/page/'.$this->uri->segment(7));
			}
		}
		else
		{
...
/* End of file blogapi.php */
/* Location: ./application/controllers/blogapi.php */